...
# Prepare combined CSV path
save_dir = f'{OUT_DIR}/{MISSING_SCENARIO}/loglik'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
combined_csv_path = f'{save_dir}/loglik_summary_all.csv'

# Initialize combined results DataFrame
if os.path.exists(combined_csv_path):
    combined_df = pd.read_csv(combined_csv_path)
else:
    combined_df = pd.DataFrame(columns=['Dataset', 'Method', 'Missing_Rate', 'Mean_LogLikelihood'])
...
for i1 in range(len(LIST_DATASETS)):
    ...
    # === ADD TO COMBINED CSV ===
    print('Updating combined Log-likelihood summary table...')
    for method, values in loglik_dict.items():
        arr = np.array(values)
        mean_loglik = np.nanmean(arr, axis=1)  # average across repeats
        for i_miss, miss_rate in enumerate(LIST_MISS_RATES):
            combined_df.loc[len(combined_df)] = {
                'Dataset': data_name,
                'Method': method,
                'Missing_Rate': miss_rate,
                'Mean_LogLikelihood': mean_loglik[i_miss]
            }

    # Save the combined table after each dataset
    combined_df.to_csv(combined_csv_path, index=False)
    print(f'Updated combined CSV -> {combined_csv_path}')
