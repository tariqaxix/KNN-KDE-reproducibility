# KNN-KDE-reproducibility
## Reproduction of "Numerical Data Imputation for Multimodal Data Sets"

### ORIGINAL RESEARCH CREDIT 
***This repository contains a reproduction of the work below. All scientific credit belongs to the original authors.***

| Detail | Information |
| :--- | :--- |
| **Paper Title** | Numerical Data Imputation for Multimodal Data Sets: A Probabilistic Nearest-Neighbor Kernel Density Approach |
| **Authors** | Lalande Florian and Doya Kenji |
| **Published In** | Transactions on Machine Learning Research (06/2023) |
| **Paper Link** | [OpenReview Forum](https://openreview.net/forum?id=KqR3rgooXb) |
